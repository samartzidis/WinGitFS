...
async Task<int> RunAsync(GitFsOptions options)
{
    ...
    try
    {
        ...
        {
            logger.LogError("The path already exists. Specify a path that does not exist so WinGitFS can create and use it.");
            return 2;
        }
...
string BuildCloneUrl(GitFsOptions options)
{
    ...
    {
        var orgUrl = options.OrganizationUrl.ToString().TrimEnd('/');
        baseUrl = $"{orgUrl}/{options.Project}/_git/{options.Repository}";
    }
    else
    {
        baseUrl = $"https://github.com/{options.Owner}/{options.Repository}.git";
    }
